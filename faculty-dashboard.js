...
window.openViewModal = function(idx) {
    ...
    const att   = loadAtt(s.roll);
    const marks = loadMarks(s.roll);
    const absent = att.total - att.present;
    const pct   = att.total ? Math.round((att.present / att.total) * 100) : null;
    const pctColor = pct === null ? '#6b7280' : pct >= 75 ? '#059669' : '#dc2626';
    const pctLabel = pct !== null ? pct + '%' : 'No data';

    document.getElementById('viewModalBody').innerHTML = `
        <div style="display:flex;flex-direction:column;gap:14px;">

            <!-- Name banner -->
            <div style="background:linear-gradient(135deg,#dbeafe,#ede9fe);border-radius:12px;padding:18px 20px;">
                <div style="font-size:20px;font-weight:800;color:#111827;">${s.name}</div>
                <div style="font-size:13px;color:#4b5563;margin-top:4px;">
                    Roll No: <strong>${s.roll}</strong> &nbsp;|&nbsp; Class: <strong>${s.div}</strong>
                </div>
            </div>

            <!-- Info grid -->
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                <div style="background:#f8faff;border-radius:10px;padding:13px 15px;border-left:4px solid #2563eb;">
                    <div style="font-size:11px;font-weight:700;color:#2563eb;margin-bottom:4px;">Contact</div>
                    <div style="font-size:14px;color:#111827;">${s.contact}</div>
                </div>
                <div style="background:#f8faff;border-radius:10px;padding:13px 15px;border-left:4px solid #7c3aed;">
                    <div style="font-size:11px;font-weight:700;color:#7c3aed;margin-bottom:4px;">Email</div>
                    <div style="font-size:13px;color:#111827;word-break:break-all;">${s.email}</div>
                </div>
                <div style="background:#f8faff;border-radius:10px;padding:13px 15px;border-left:4px solid #d97706;">
                    <div style="font-size:11px;font-weight:700;color:#d97706;margin-bottom:4px;">Parent / Guardian</div>
                    <div style="font-size:14px;color:#111827;">${s.parent || '—'}</div>
                </div>
                <div style="background:#f8faff;border-radius:10px;padding:13px 15px;border-left:4px solid ${pctColor};">
                    <div style="font-size:11px;font-weight:700;color:${pctColor};margin-bottom:4px;">Attendance</div>
                    <div style="font-size:22px;font-weight:800;color:${pctColor};">${pctLabel}</div>
                    <div style="font-size:11px;color:#6b7280;">${att.present} present / ${absent} absent (${att.total} total)</div>
                </div>
            </div>
            ...
            ${marks.length ? `
            <div>
                <div style="font-size:12px;font-weight:700;color:#2563eb;text-transform:uppercase;letter-spacing:.8px;margin-bottom:8px;">Recent Marks</div>
                ${marks.slice(-4).map(m => `
                    <div style="display:flex;justify-content:space-between;align-items:center;
                                padding:9px 14px;background:#f8faff;border-radius:8px;margin-bottom:6px;font-size:13px;">
                        <span style="color:#111827;font-weight:600;">${m.subject}</span>
                        <span style="font-weight:700;color:#2563eb;">${m.marks} / 100</span>
                        <span style="background:#dbeafe;color:#1e40af;padding:2px 10px;
                                     border-radius:20px;font-size:11px;font-weight:700;">${m.grade}</span>
                    </div>`).join('')}
            </div>` : `
            <div style="text-align:center;color:#9ca3af;font-size:13px;padding:8px 0;">
